/**
 * Audio Tutor PWA - All-in-One Cloudflare Worker + Edge-TTS Proxy
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    // 2. 判斷是否為 Edge-TTS API 請求 (/api/tts, /tts, /v1/audio/speech)
    if (url.pathname === '/api/tts' || url.pathname === '/tts' || url.pathname === '/v1/audio/speech') {
      return handleEdgeTts(request, url);
    }

    // 3. 其他靜態資源請求交由 Cloudflare ASSETS 處理 (若有配置 assets binding)
    if (env && env.ASSETS) {
      try {
    ...
    return new Response('Audio Tutor Edge-TTS Worker is Running. Use /api/tts to synthesize speech.', {
      status: 200,
...
async function handleEdgeTts(request, url) {
  let text = '';
